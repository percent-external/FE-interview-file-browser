...
import NameFilterComponent from "./components/NameFilterComponent";
...
function DataGrid() {
  ...
  const [sizeFilter, setSizeFilter] = React.useState(200);
  const [minMaxFilter, setMinMaxFilter] = React.useState('greaterThan');
  ...
  const {data, loading, error} = useListEntriesQuery({
    variables: {
      ...
      where: {
         ...
         */
        size_gt: minMaxFilter === 'greaterThan' ? sizeFilter : null, // Int
        size_lt: minMaxFilter === 'lessThan' ? sizeFilter : null,
  ...
  const handleMinMaxChange = (minMaxValue: string) => {
    setMinMaxFilter(minMaxValue);
  }
        ...
        <Paper>
          <Toolbar>
            <Box display="flex" alignItems="center"
              ...
              <Box>
                <NameFilterComponent onNameChange={handleNameChange}/>
                <TypeFilterComponent onTypeChange={handleTypeChange}/>
                <SizeFilterComponent
                  sizeNumber={sizeFilter}
                ...
                {rows.map(({path, __typename, name, size, id}) => {
                  ...
                  return (
                    <TableRow key={id}>
                      ...
                      <TableCell
                        align="right">{isUpDir ? '_' : name}</TableCell>
                      <TableCell
                        align="right">{isUpDir ? '_' : __typename}</TableCell>
                      <TableCell
                        align="right">{isUpDir ? '_' : size}</TableCell>
                    </TableRow>
                  )
                })}
              </TableBody>
