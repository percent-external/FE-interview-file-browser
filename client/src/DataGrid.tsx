...
import {
  Box, Button, Chip, Paper, Table, TableBody, 
...
function DataGrid() {
  ...
  const { data, loading, error } = useListEntriesQuery({
    variables: {
      ...
      where: {
         ...
         */
        name_contains: search.toLocaleLowerCase(),
  ...
  const handleDelete = () => setSizeGt(0);

  const handleDirectorySelection = () => setDirectory(!directory);

  const handleFileSelection = () => setFileBox(!fileBox);
