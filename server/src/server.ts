...
const filterEntries = (entries: any[], where?: Maybe<WhereInput>): any[] => {
  if (where) {
    return compact(
      map(entries, (entry) => {
        let tmpEntry: FileSystemEntry | undefined = lookupPath(
          entry.path.split("/")
        );
        ...
        // Type
        if (where.type_eq) {
          if (where.type_eq === "Directory" && tmpEntry?.isFile)
            tmpEntry = undefined;
          if (where.type_eq === "File" && tmpEntry?.isDirectory)
            tmpEntry = undefined;
        }
        ...
        if (where.name_contains) {
          if (!tmpEntry?.name.toLowerCase().includes(where.name_contains))
            tmpEntry = undefined;
        }
